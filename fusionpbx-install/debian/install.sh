...
perl -pi -e "s/\/etc\/ssl\/certs\/nginx.crt;/\/etc\/letsencrypt\/live\/${Domain}\/cert.pem;/g" /etc/nginx/sites-available/fusionpbx
perl -pi -e "s/\/etc\/ssl\/certs\/nginx.key;/\/etc\/letsencrypt\/live\/${Domain}\/privkey.pem;/g" /etc/nginx/sites-available/fusionpbx
#Postgres
...
echo "   https://$server_address or https://${Domain}"
echo ""
echo ""
